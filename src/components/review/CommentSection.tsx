...
import { useCreateReviewMutation, useGetReviewsByIssueQuery } from "@/redux/features/reviews/reviewApi";
...
const CommentSection = ({ issueId }: CommentSectionProps) => {
  ...
  const { data, isLoading, isError } = useGetReviewsByIssueQuery({
    issueId,
    page: currentPage,
    limit: 10,
  });
...
const getUserAvatar = () => {
    return user?.avatar?.url || userIcon;
  };
